...
import { LayoutDashboard, Code2, FolderKanban, ShoppingCart, User as UserIcon, ShieldAlert, Plus, Layers } from 'lucide-react';
...
const MobileNav: React.FC<MobileNavProps> = ({ path, mode, user, navigateTo }) => {
  return (
    <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-[500] pointer-events-none w-full max-w-[360px] px-4">
      <div className="bg-black/60 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] flex items-center justify-around p-2 pointer-events-auto shadow-[0_20px_50px_rgba(0,0,0,0.5)] ring-1 ring-white/5">
        <button 
          ...
          className={`relative p-3.5 transition-all rounded-full ${path === '/dashboard' && mode !== AppMode.EDIT ? 'text-white' : 'text-zinc-600'}`}
        >
        ...
        <button 
          ...
          className={`relative p-3.5 transition-all rounded-full ${path === '/projects' ? 'text-white' : 'text-zinc-600'}`}
        >
        ...
        <button 
          ...
          className={`relative p-3.5 transition-all rounded-full ${path === '/shop' ? 'text-white' : 'text-zinc-600'}`}
        >
        ...
        <button 
          ...
          className={`relative p-3.5 transition-all rounded-full ${path === '/profile' ? 'text-white' : 'text-zinc-600'}`}
        >
