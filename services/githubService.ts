...
export class GithubService {
  ...
  async pushToGithub(config: GithubConfig, files: Record<string, string>, appConfig?: ProjectConfig, customMessage?: string) {
    ...
    // Ensure at least a dummy file exists in app if it's empty to prevent cp error in workflow
    const hasAppFiles = Object.keys(files).some(k => k.startsWith('app/'));
    if (!hasAppFiles) {
      allFiles['app/README.md'] = "# Main App Workspace\nGenerated by OneClick Studio.";
    }
